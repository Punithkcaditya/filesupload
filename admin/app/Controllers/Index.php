...
class Index extends BaseController
{
    ...
    public function updateemployeedetails(){
        if ($this->request->getMethod() == "post") {
            ...
            $udata["ssjl_isdisapproved"] = 2;
            $ssjl_resume = $this->request->getFile('ssjl_resume');
            $ssl_profile_pic = $this->request->getFile('ssl_profile_pic');
            $ssjl_adhaar_copy = $this->request->getFile('ssjl_adhaar_copy');
            $ssjl_releiving_letter = $this->request->getFile('ssjl_releiving_letter');
            $ssjl_education_documents = $this->request->getFile('ssjl_education_documents');
            $ssjl_pan_copy = $this->request->getFile('ssjl_pan_copy');
            $ssjl_previous_pay_slips = $this->request->getFile('ssjl_previous_pay_slips');
            $ssjl_rent_agreement = $this->request->getFile('ssjl_rent_agreement');
            $ssjl_resumeupdated =  $this->savefilesuploaded($ssjl_resume);
            $ssl_profile_picupdated =  $this->savefilesuploaded($ssl_profile_pic);
            $ssjl_adhaar_copyupdated =  $this->savefilesuploaded($ssjl_adhaar_copy);
            $ssjl_releiving_letterupdated =  $this->savefilesuploaded($ssjl_releiving_letter);
            $ssjl_education_documentsupdated =  $this->savefilesuploaded($ssjl_education_documents);
            $ssjl_pan_copyupdated =  $this->savefilesuploaded($ssjl_pan_copy);
            $ssjl_previous_pay_slipsupdated =  $this->savefilesuploaded($ssjl_previous_pay_slips);
            $ssjl_rent_agreementupdated =  $this->savefilesuploaded($ssjl_rent_agreement);
            if(!empty($ssjl_resumeupdated)){$udata["ssjl_resume"] = $ssjl_resumeupdated;}
            if(!empty($ssl_profile_picupdated)){$udata["ssl_profile_pic"] = $ssl_profile_picupdated;}
            if(!empty($ssjl_adhaar_copyupdated)){$udata["ssjl_adhaar_copy"] = $ssjl_adhaar_copyupdated;}
            if(!empty($ssjl_releiving_letterupdated)){$udata["ssjl_releiving_letter"] = $ssjl_releiving_letterupdated;}
            if(!empty($ssjl_education_documentsupdated)){$udata["ssjl_education_documents"] = $ssjl_education_documentsupdated;}
            if(!empty($ssjl_pan_copyupdated)){$udata["ssjl_pan_copy"] = $ssjl_pan_copyupdated;}
            if(!empty($ssjl_previous_pay_slipsupdated)){$udata["ssjl_previous_pay_slips"] = $ssjl_previous_pay_slipsupdated;}
            if(!empty($ssjl_rent_agreementupdated)){$udata["ssjl_rent_agreement"] = $ssjl_rent_agreementupdated;}
            $update = $this->introductionform_model->where("ssjl_employee_id", $employee_id)->set($udata)
            ->update();
                if ($update) {
...
public function savefilesuploaded($ssjl_files){
  if ($ssjl_files->isValid() && !$ssjl_files->hasMoved()) {
                    ...
                    //$ssjl_files->move(BASEPATH . 'admin/uploads', $ssjl_files_pic);
...
public function savefilesuploadedold($ssjl_files){
 if ($ssjl_files->isValid() && !$ssjl_files->hasMoved()) {
...
// view pdf files
    ...
    {
			 $session = session();
		$filename_without_ext = pathinfo($filename, PATHINFO_EXTENSION);
		if($filename_without_ext=='pdf'){
			...
			die($content);
		}else if($filename_without_ext=='doc'){
			$filepath = base_url() . '/uploads/' . $filename;
			$content = file_get_contents($filepath);
			header('Content-Type: application/msword'); // change content type to doc
			header('Content-Length: ' . strlen($content));
			header('Content-Disposition: inline; filename="' . $filename . '"');
			header('Cache-Control: private, max-age=0, must-revalidate');
			header('Pragma: public');
			...
			$session->setFlashdata("error", "File Type Not Supported");
    ...
    public function reject()
    {
        ...
        if ($this->request->getMethod() == "post") {
        extract($this->request->getPost());
        $udata["ssjl_isdisapproved"] = 1;
        $udata["ssjl_comment"] = $ssjl_comment;
        $update = $this->introductionform_model
        ->where("ssjl_into_id", $ssjl_into_id)
        ->set($udata)
        ->update();
        if ($update) {
        $session->setFlashdata(
